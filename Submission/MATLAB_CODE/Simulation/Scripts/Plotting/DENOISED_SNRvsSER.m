...
title("(A)")
hold on;    
semilogy(LOG(AveragedMSE_LS_DENOISED), '-d');
semilogy(LOG(AveragedMSE_LS),          '-*');
semilogy(LOG(AveragedMSE_MMSE),        '-^');
semilogy(LOG(AveragedMSE_LSTM),        '-s');
legend("Denoised LS", "LS", "MMSE", "LSTM");
subplot(2,1,2)
title("(B)")
hold on;  
semilogy(LOG(AveragedMSE_LS_DENOISED(1:20)), '-d');
semilogy(LOG(AveragedMSE_LS(1:20)),          '-*');
semilogy(LOG(AveragedMSE_MMSE(1:20)),        '-^');
semilogy(LOG(AveragedMSE_LSTM(1:20)),        '-s');
legend("Denoised LS", "LS", "MMSE", "LSTM");

%% MSE OF NOISE PLOT
figure();
hold on;
semilogy(LOG(MSE_NOISY_RUN), '-o');
semilogy(LOG(MSE_CLEAN_RUN), '-s');
...
hold on;
hold on;
semilogy(LOG(AveragedSER_LS_DENOISED), '-d');
semilogy(LOG(AveragedSER_LS),          '-*');
semilogy(LOG(AveragedSER_MMSE),        '-^');
semilogy(LOG(AveragedSER_LSTM),        '-s');
semilogy(LOG(AveragedSER_COHERENT),    '-o');
legend("Denoised LS", "LS", "MMSE", "LSTM", "Ideal Channel");
subplot(2,1,2)
title("(B)")
hold on;
semilogy(LOG(AveragedSER_LS_DENOISED(1:20)), '-d');
semilogy(LOG(AveragedSER_LS(1:20)),          '-*');
semilogy(LOG(AveragedSER_MMSE(1:20)),        '-^');
semilogy(LOG(AveragedSER_LSTM(1:20)),        '-s');
semilogy(LOG(AveragedSER_COHERENT(1:20)),    '-o');
legend("Denoised LS", "LS", "MMSE", "LSTM", "Ideal Channel");
