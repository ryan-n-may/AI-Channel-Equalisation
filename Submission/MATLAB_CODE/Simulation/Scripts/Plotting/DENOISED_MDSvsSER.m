...
for j = 1:AVERAGEING_ROUNDS
    ...
    for i = 1:size(MDS, 2)
        ...
        %% MSE
        MSE_LS          = [MSE_LS,          CalculateMSE(CHANNEL_H_Complex,    H_LS_COMPLEX)];
        MSE_LS_DENOISED = [MSE_LS_DENOISED, CalculateMSE(CHANNEL_H_Complex,    H_LS_DENOISED_COMPLEX)];
        MSE_LSTM        = [MSE_LSTM,        CalculateMSE(CHANNEL_H_Complex,    O_LSTM_COMPLEX)];   
        MSE_MMSE        = [MSE_MMSE,        CalculateMSE(CHANNEL_H_Complex,    H_MMSE_C)];
        ...
        H_LS_COMPLEX = H_LS_COMPLEX + 2+2i;(1:10)
...
title("(1)")
hold on;
plot((AveragedSER_LS_DENOISED), '-d');
plot((AveragedSER_LS),          '-*');
plot((AveragedSER_MMSE),        '-^');
plot((AveragedSER_LSTM),        '-s');
legend("Denoised LS", "LS", "MMSE", "LSTM");

subplot(2,1,2)
title("(2)")
hold on;
plot((AveragedSER_LS_DENOISED(1:5)), '-d');
plot((AveragedSER_LS(1:5)),          '-*');
plot((AveragedSER_MMSE(1:5)),        '-^');
plot((AveragedSER_LSTM(1:5)),        '-s');
legend("Denoised LS", "LS", "MMSE", "LSTM");
